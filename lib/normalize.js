  ...
  function normalize(depsPerDir, inconsistencies) {
    ...
    function normalizeOne(i) {
      ...
      commander.prompt('(' + (i+1) + '/' + deps.length + ') New version for `' + deps[i] + '` [' + inconsistencies[deps[i]].sort().join(', ') + ']: ', function(version) {
        if (!version) {
          console.log('Skipping dependency `' + deps[i] + '`.');
    ...
    if (deps.length > 0) {
      console.log(deps.length + ' dependencies need normalization. (Leave a dependency empty to skip it.)')
      normalizeOne(0);
    } else {
      console.log('All dependencies already normalized.')
      callback && callback();
    }
  }
}
